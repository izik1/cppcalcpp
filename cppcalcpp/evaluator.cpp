...
int64_t evaluator::evaluateOp(const std::shared_ptr<exprtree> lhs, int64_t rhs, type op) {
    switch(op) {
    case type::plus: return evaluate(lhs) + rhs;
    case type::minus: return evaluate(lhs) - rhs;
    case type::astrisk: return evaluate(lhs) * rhs;
    case type::slash: return evaluate(lhs) / rhs;
    case type::carrot: return ipow(evaluate(lhs), rhs);
    case type::equals:
